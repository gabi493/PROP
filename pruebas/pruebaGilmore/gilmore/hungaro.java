...
public class hungaro{
    ...
    public int[][] assignaciones(double[][] matrix) {
        ...
        inicio(matrix, starsByRow, starsByCol);
        cubre_con_zero(starsByCol,coveredCols);

        while (!todo_cubierto(coveredCols)) {

            int[] primedZero = primera_solucion_sin_ceros(matrix, primesByRow, coveredRows, coveredCols);

            while (primedZero == null) {
                // keep making more zeroes until we find something that we can prime (i.e. a zero that is uncovered)
                haz_zeros(matrix,coveredRows,coveredCols);
                primedZero = primera_solucion_sin_ceros(matrix, primesByRow, coveredRows, coveredCols);
            }
            ...
            if (-1 == columnIndex){
                ...
                incremento_los_zeroes(primedZero, starsByRow, starsByCol, primesByRow);
    ...
    private boolean todo_cubierto(int[] coveredCols) {
     ...
     * @param coveredCols
     */
    private void cubre_con_zero(int[] starsByCol, int[] coveredCols) {
        for (int i = 0; i < starsByCol.length; i++) {
     ...
     * @param primesByRow
     */
    private void incremento_los_zeroes(int[] unpairedZeroPrime, int[] starsByRow, int[] starsByCol, int[] primesByRow) {
